...
class CognitiveKnowledgeRetrieval:
    ...
    def __init__(self, knowledge_base_path: str):
        ...
        logger.info(f"💾 Cache: {'Redis' if REDIS_AVAILABLE and isinstance(self.cache, redis.Redis) else 'Local Dict'}")
        logger.info(f"🕸️ Knowledge graph: NetworkX")
    ...
    def _build_sample_knowledge_graph(self):
        ...
        concepts = [
            "flood_risk", "wildfire_risk", "property_age", "foundation", "roof",
            "elevation_certificate", "defensible_space", "underwriting_guidelines",
            "property_eligibility", "risk_assessment", "insurance_premium"
        ]
        ...
        logger.info(f"🕸️ Built knowledge graph with {len(concepts)} nodes and {len(relationships)} relationships")
    ...
    def _understand_query(self, query: str, context: Dict[str, Any], related_concepts: List[str] = None) -> str:
        """
        ...
        if related_concepts:
            concept_terms = " ".join([concept.replace('_', ' ') for concept in related_concepts[:3]])  # Limit to top 3
    ...
    def _intelligent_ranking(self, results: List[Dict], query: str, context: Dict[str, Any]) -> List[KnowledgeChunk]:
        """
    ...
    def learn_from_feedback(self, query: str, results: List[KnowledgeChunk], feedback: Dict[str, Any]):
        """
    ...
    def get_intelligence_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "cache": {
                "type": "Redis" if REDIS_AVAILABLE and isinstance(self.cache, redis.Redis) else "Local Dict",
                "status": "connected" if REDIS_AVAILABLE and isinstance(self.cache, redis.Redis) else "local"
            },
            "knowledge_graph": {
                "type": "NetworkX",
